		...
		success: function( data ){
			...
			jQuery.each(data, function( i, value ){
				if( value.status != 'hide' && value.status != 'deleted' ){
					table.append('<tr><td>' + value.custom_id + '</td><td>' + value.title + '</td><td>' + value.description + '</td><td>' + value.status + '</td><td>' + value.add_date + '</td><td>' + value.last_change_date + '</td><td> <form class="orderChangeForm addChangeOrder"><input type="hidden" name="id" value="' + value.id + '" /><input type="hidden" name="custom_id" value="' + value.custom_id + '" /><input type="hidden" name="title" value="' + value.title + '" /><input type="hidden" name="description" value="' + value.description + '" /><input type="hidden" name="status" value="' + value.status + '" /><input type="hidden" name="add_date" value="' + value.add_date + '" /><input type="hidden" name="last_change_date" value="' + value.last_change_date + '" /><input type="hidden" name="action" value="change" /><input type="submit" value="Изменить" class="btnshowcart" /></form> </td>				<td> <form class="orderHideForm addChangeOrder"><input type="hidden" name="id" value="' + value.id + '" /><input type="hidden" name="action" value="hide" /><input type="submit" value="x Скрыть" class="btnshowcart" /></form> </td></tr>');
				}
			});
...
function showMessege( text ){
    ...
    jQuery('#orderStatusRequest').fadeIn(900).fadeOut(900, function(){jQuery('#orderStatusRequest').remove()});
}

jQuery(document).ready(function(){
	jQuery('form.addChangeOrder').submit(submitOrderForm);
	printOrderList( );
});
