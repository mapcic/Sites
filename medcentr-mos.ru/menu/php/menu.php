...
// 		if (!empty($child)) {
// 			$out = $out.'<div>'.$val->title.'</div>'.printChild($arr, $num, $id);
// 		} else {
...
function printChild(&$arr, $num, $id) {
	...
	foreach ($child as $key => $val) {
		$out = $out.'<div>'.$val->title.'</div>'.printChild($arr, $num+1, $val->id);
	}
...
foreach ($menus as $menu) {
	...
	foreach ($menuByLevel[$levelOffset] as $key => $val) {
		$out = '<div class="father">'.$val->title.'</div>'.printChild($menuByLevel, $levelOffset+1, $val->id);
		echo $out;
